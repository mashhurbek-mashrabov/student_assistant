...
class BaseController:
    def __init__(self, message, bot: telebot.TeleBot):
        ...
        self.user = TutorTelegramUser.objects.get_or_create(chat_id=self.chat_id)[0]
    ...
    def set_step(self, step: TutorBotSteps):
        user = self.user
        user.step = step
        user.save()
    ...
    def main_menu_inline_button(self):
        return InlineKeyboardButton(self.t('main menu'), callback_data=TutorCallbackData.main_menu_button)
    ...
    def back_inline_button(self):
        return InlineKeyboardButton(self.t('back button'), callback_data=TutorCallbackData.back_button)
    ...
    def skip_inline_button(self):
        return InlineKeyboardButton(text=self.t('skip'), callback_data=TutorCallbackData.skip)
